...
def _localize_action(action: str, zone_id: str) -> str:
    ...
    for uk_term, local_term in sorted(_INDIA_SUBS.items(), key=lambda x: -len(x[0])):
        result = _replace_term(result, uk_term, local_term)
    ...
    for length in range(len(parts), 0, -1):
        ...
        if prefix in _ZONE_LOCALE:
            for old, new in sorted(_ZONE_LOCALE[prefix].items(), key=lambda x: -len(x[0])):
                result = _replace_term(result, old, new)
            break
...
def _check_pesticide(pesticide: dict[str, Any], T: dict) -> list[dict]:
    ...
    if days is not None and ppm is not None and days <= 3 and ppm >= T["pesticide_ppm_warning"]:
        anomalies.append(_anomaly(
    ...
    # Type-specific alert
    if ppm is not None and ptype == "neonicotinoid" and ppm > 2.0:
        anomalies.append(_anomaly(
...
def _check_visitation(visitation: dict[str, Any], T: dict) -> list[dict]:
    anomalies = []
    ratio     = visitation.get("visitation_ratio")
    decline   = visitation.get("decline_rate_12w")
    timing    = visitation.get("pollination_timing_disruption")
    flowering = visitation.get("flowering_success_rate")
    visits    = visitation.get("avg_visitations_per_hour")
    ...
    if bare_soil is not None:
        if bare_soil >= T["bare_soil_critical"]:
            ...
            anomalies.append(_anomaly(
                ...
                f"(which include the majority of wild solitary bee species) require stable, "
                f"undisturbed soil structure to complete their nesting cycle.",
