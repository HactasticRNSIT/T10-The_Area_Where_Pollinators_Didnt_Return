...
def test_latest_country_pesticide_use_reads_bundled_dataset():
    ...
    assert india["year"] == 2023
    assert india["tonnes"] == 40094.0

    assert get_latest_country_pesticide_use("USA")["tonnes"] == 429501.0
    assert get_latest_country_pesticide_use("BRA")["tonnes"] == 800652.0
    assert get_latest_country_pesticide_use("CHN")["tonnes"] == 217996.0
...
def test_fixed_indian_state_codes_resolve_state_adjustments():
    ...
    for zone_id, state_demand in expected.items():
        ...
        assert pesticide["country_pesticide_year"] == 2023
        assert pesticide["country_pesticide_tonnes"] == 40094.0
        assert pesticide["state_demand_mt_reference"] == state_demand
...
def test_zone_baselines_preserve_scoring_interface():
    ...
    assert india_state["state_demand_mt_reference"] == 2100.0
    ...
    assert usa_farm["country_pesticide_tonnes"] == 429501.0
